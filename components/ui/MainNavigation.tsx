...
const NavBar = () => {
	...
	return (
		<header
			className={`w-screen fixed z-20  ${bg || isNavOpen ? 'bg-black text-gray-500' : ''} transition duration-500 h-14 flex items-center`}
		>
			<nav className="flex justify-between lg:grid lg:grid-cols-nav text-xs font-bold h-10 w-full items-center">
				{/* Burger menu */}
				<section className="flex lg:hidden items-center z-20 pb-2">
					...
					>
						...
						></span>
						<span
							className={`${genericHamburgerLine} ${isNavOpen ? "opacity-0" : ""
								}`}
						></span>
						<span
							className={`${genericHamburgerLine} ${isNavOpen
